...
def test_белый_список_сужает_перечисление(monkeypatch):
    """Первые прогоны на боевой базе идут по паре сетей, а не по миллиарду
    строк. Список задан — источник не спрашиваем вовсе."""
    monkeypatch.setenv("ONLY_CHAINS", "пятёрочка, ПЕРЕКРЁСТОК")
    assert known_chains() == ["ПЕРЕКРЁСТОК", "ПЯТЁРОЧКА"]


def test_белый_список_не_отменяет_исключённые(monkeypatch):
    """Исключённые сети запрещены безусловно: попади «КАРУСЕЛЬ» в белый
    список по недосмотру, она всё равно не должна доехать до витрины."""
    monkeypatch.setenv("ONLY_CHAINS", "МАГНИТ,КАРУСЕЛЬ")
    assert known_chains() == ["МАГНИТ"]
