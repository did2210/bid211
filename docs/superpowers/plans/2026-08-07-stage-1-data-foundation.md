...
def generate(dsn: str, rows: int, year: int = 2026, seed: int = 42) -> dict[str, int]:
    ...
    for chain, _ in CHAINS:
        for i in range(220):
            ...
            stores.append((f"{chain[:3]}{i:05d}", chain,
...
def test_читается_csv_с_нужным_числом_колонок():
    first = next(iter(read_chunk(Chunk(2026, 7, "МАГНИТ")))).split(b"\n")[0]
    assert first.count(b",") == 7, "восемь колонок — семь запятых"
...
def test_флаг_опта_превращается_в_ноль_или_единицу():
    body = b"".join(read_chunk(Chunk(2026, 7, "МАГНИТ")))
    opts = {line.split(b",")[7] for line in body.strip().split(b"\n")}
    assert opts <= {b"0", b"1"}
```
...
def test_расхождение_отменяет_подмену(monkeypatch):
    ...
    from gfd_sync.reader import Stats
    from decimal import Decimal
...
def test_заливка_в_теневую_не_видна_в_основной():
    """Полная перезаливка идёт мимо боевой таблицы: пока она не кончится,
    люди работают на старых данных."""
    from gfd_sync.loader import create_shadow, swap_shadow
    ch = ch_client()
...
def test_обмен_таблиц_переключает_данные_разом():
    from gfd_sync.loader import create_shadow, swap_shadow
    ch = ch_client()
    create_shadow(ch)
    ch.command("TRUNCATE TABLE sales_shadow")
